          ...
          >
            ...
            >
              <ul class="absolute sub-nav top-full -mx-4" v-if="list.show">
                <li
                  class="p-4 my-1 bg-primary relative sub-item w-full"
                  v-for="(vv, kk) in list.list"
                  :key="kk"
                >
                  <router-link :to="vv.link" class="nav-link w-full text-center">{{vv.label}}</router-link>
                </li>
              </ul>
            </transition>
          </li>
          ...
          <template v-for="(menuItem, index) in navList" :key="index">
            <router-link :to="menuItem.link" class="nav-link">
              <q-item clickable :active="menuItem.label === 'Outbox'" v-ripple>
                ...
                <q-item-section class="text-secondary">{{ menuItem.label }}</q-item-section>
                <q-separator />
              </q-item>
            </router-link>
          </template>
