...
<div class="lm-about">
    ...
    <section class="lm-culture-points lm-light-dark-background">
        ...
        <div class="lm-culture-team row justify-center pb-14 lg:container lg:mx-auto">
            <div class="team-container col-9 col-xs-12 col-md-9 flex items-center pb-24 q-px-xs-md q-px-md-none" :class="[k !== 0 ? 'pt-24' :'', k === teamData.length - 1 ? 'last' : '']" v-for="(v, k) in teamData" :key="k" :ref="
              (el) => {
                if (el) itemRefs[k] = el;
              }
            ">
    ...
    setup() {
      ...
      onMounted(() => {
        ...
        if (typeof id !== 'undefined') {
          let index = Number(id);
          const currentRef = <InstanceType<typeof HTMLElement>>itemRefs.value[index];
          setTimeout(() => currentRef.scrollIntoView({behavior: 'smooth'}), 1000);
        }
      });
      return {
          itemRefs,
