                            ...
                            <div class="row justify-center items-center mb-10">
                                <div class="col-xs-12 col-sm-12 col-md-5 left q-py-xs-lg q-py-xs-lg">
                                    <div class="mt-2 mb-20 sm:mb-0">
                                        <h2 class="subtitle text-quaternary text-weight-bolder text-5xl pb-10 pr-5">{{ v.subTitle }}</h2>
                                        <p class="description text-white text-sm pr-0 sm:pr-24 text-weight-thin leading-7 tracking-wide ">{{ v.description }}</p>
                                    </div>
            ...
            <div class="lm-container row justify-center">
                <div class="col-md-9 col-xs-11">
                    <div class="lm-product-points mb-10 sm:mb-20 mt-10 sm:mt-20">
                        ...
                        <div class="text-center text-weight-thin text-xs leading-7" v-for="(v, k) in productPoints[0].description" :key="k">{{ v }}</div>
                    </div>
                </div>
                ...
                <div class="col-12 row relative justify-center">
                    <div class="col-md-4 col-xs-11 flex flex-col justify-center">
                        <div class="lm-product-points mb-10 sm:mb-20">
                            ...
                            <div class="text-center text-weight-thin text-xs leading-7" v-for="(v, k) in productPoints[1].description" :key="k">{{ v }}</div>
                        </div>
                        <div class="lm-product-points mb-10 sm:mb-0">
                            ...
                            <div class="text-center text-weight-thin text-xs leading-7" v-for="(v, k) in productPoints[3].description" :key="k">{{ v }}</div>
                        </div>
                    </div>
                    <div class="col-xs-11 col-md-4">
                        <q-img class="lm-product-img" src="product-intro-black.svg" fit="contain">
                            <div class="lm-product-container absolute-full text-weight-bolder flex flex-center transparent text-7xl">
                    ...
                    <div class="col-md-4 col-xs-11 flex flex-col justify-center">
                        <div class="lm-product-points mb-10 sm:mb-20">
                            <div class="text-center text-weight-bolder text-2xl mb-2 mt-10 sm:mt-0">{{ productPoints[2].title }}</div>
                            <div class="text-center text-weight-thin text-xs leading-7" v-for="(v, k) in productPoints[2].description" :key="k">{{ v }}</div>
                        </div>
                        <div class="lm-product-points">
                            <div class="text-center text-weight-bolder text-2xl mb-2">{{ productPoints[4].title }}</div>
                            <div class="text-center text-weight-thin text-xs leading-7" v-for="(v, k) in productPoints[4].description" :key="k">{{ v }}</div>
                        </div>
                    </div>
                </div>
                <div class="col-12">
                    <div class="mt-8 relative row justify-center">
                        <div class="w-full col-7 relative top-0 sm:-top-36">
                            <q-img class="w-full" src="service-progress.png" fit="contain" height="34rem" :style="{ opacity: 0.2 }"></q-img>
                        </div>
                        <div class="col-12 absolute top-0">
                            <div class="lm-product-points">
                                <div class="text-center text-weight-bolder text-2xl mb-2">{{ productPoints[5].title }}</div>
                                <div class="text-center text-weight-thin text-xs leading-7" v-for="(v, k) in productPoints[5].description" :key="k">{{ v }}</div>
                            </div>
                            <div class="lm-source-extra mt-16">
                                <div class="text-center text-weight-bolder text-2xl mb-2">{{ sourceInfo.title }}</div>
                                <div class="text-center text-weight-bolder text-base leading-10" v-for="(v, k) in sourceInfo.description" :key="k">{{ v }}</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <section class="transparent mb-20">
        <div class="lg:container lg:mx-auto">
            <div class="lm-service-advantage row justify-center p-10">
                ...
                <div :class="['col-12 row py-20 item', k % 2 !== 0 ? 'reverse' : '']" v-for="(v, k) in serviceAdvantage.list" :key="k" :ref="(el) => {
                if (el) itemRefs[k] = el;
            }">
                    <div class="col-xs-12 col-md-7">
                        ...
                        <div class="text-white text-weight-thin text-base leading-6 mb-5 sm:mb-0">{{ v.description }}</div>
                    </div>
...
export default {
    ...
    setup() {
        ...
        const serviceContent = {
            ...
            list: [{
                    ...
                    list: [{
                        ...
                        {
                            title: ['选课', '指导'],
                            description: ['结合个人优势与官方要求', '制定个性化选课方案'],
                        },
                        {
                            title: ['学术', '衔接'],
                            description: ['根据学生的专业和所选课程', '提前预习，做好准备。'],
                        },
                    ],
                },
                {
        ...
        const productPoints = [{
                title: '各行业职业精英',
                description: ['独家深度研习课程导师', '全面覆盖热门行业与高科技领域'],
            },
            {
                title: '理想考研院校导师',
                description: [
            ...
            {
                title: '双语教育顾问',
                description: ['实时监控课程进度', '老师—学生—家长', '高效沟通桥梁'],
            },
            {
                title: '专业科目学辅导老师',
                description: [
            ...
            {
                title: '学术教授',
                description: ['量身定制学术进阶课程', '辅导学术', '保证高GPA'],
            },
            {
                title: '同校学长学姐',
                description: ['过来人全方位领航', '帮助新生快速融入', '开启高能状态'],
            },
        ];
        const productIntro = [
