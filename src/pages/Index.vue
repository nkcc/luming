...
<q-page>
    <section class>
        <q-img class="lm-background" src="lb.jpg" loading="eager" :no-spinner="true" :img-class="bgFilter">
            <div class="lm-introduction row justify-center">
                <div class="lm-text-title text-center lg:container lg:mx-auto animate__animated animate__fadeIn">
                    <span>鹿名国际教育</span>
                </div>
                <div class="lm-text-second-title animate__animated animate__fadeInUp animate__delay-1s">
                    <q-img class="logo-title" src="logo_letter.svg" fit="contain" loading="eager" :no-spinner="true"></q-img>
                </div>
                <p class="text-center animate__animated animate__fadeInUp animate__delay-2s">让世界名校， 不再高不可攀</p>
            </div>
            <div class="show-more justify-center row animate__animated animate__slow animate__repeat animate__infinite animate__slideOutUp" @click="scrollFullScreen">
                <q-img class="up-scroll" src="up_arrow.svg" fit="contain" height="3.125rem" width="12.5rem" loading="eager" :no-spinner="true" />
            </div>
        </q-img>
    </section>
    <service></service>
                        ...
                        <q-carousel-slide :name="'carousel' + k" class="column no-wrap">
                            <div class="row justify-center items-center">
                                <div class="col-xs-12 col-sm-12 col-md-5 left">
                                    <h2 class="text-secondary text-weight-bold text-6xl pb-3 sm:pb-14 sm:text-6xl title">{{ v.title }}</h2>
                                    <div class="content">
                                        <h3 class="text-white text-weight-bold sm:mt-0 text-4xl pb-0 sm:pb-10 sm:text-5xl">{{ v.subTitle }}</h3>
                                ...
                                <div class="col-xs-12 col-sm-12 col-md-4 right mt-5 sm:mt-0">
                                    <q-responsive :ratio="isMobile ? 376 / 183 : 692 / 511">
                                        <q-img class="points-img" :src="v.imgLink" loading="eager" :no-spinner="true" fit="cover" position="center top"></q-img>
                                    </q-responsive>
                                </div>
                                <div class="col-xs-12 mt-12 lg:mt-0 lt-md">
                                    <p class="description text-white text-xs sm:text-xl leading-6">{{ v.description }}</p>
                                </div>
                            </div>
                        </q-carousel-slide>
                    </template>
    ...
    <section class="lm-culture lg:container lg:mx-auto">
        <div class="caption">
            <q-img class="logo-letter" src="logo_letter_dark.svg" fit="contain" loading="eager" :no-spinner="true"></q-img>
        </div>
        <div class="row q-col-gutter-sm justify-center">
            ...
            <div class="col col-sm-6 col-xs-12 col-md-3 culture-item gt-sm" v-for="(v, k) in cultureData" :key="k" :data-id="k" v-intersection.once="onCultureIntersection">
                <transition appear :enter-active-class="cultureAnimation()">
                    <div class="animation-container" v-if="cultureData[k].visible">
            ...
            <div class="offer-container">
                <div class="row">
                    <div class="col-12 q-pa-xs-xs mb-14">
                        ...
                        <ul class="area">
                            <li :class="['label', slideOffer === v.type ? 'active' : '']" v-for="(v, k) in offerType" :key="k" @click="changeSlideOfferType(v.type, k)">{{ v.title }}</li>
                        </ul>
                    </div>
                    <div class="col-12">
...
const cultureData = ref([{
        ...
        description: ['鹿名国际教育成立于2014年美国波士顿，其使命致力于让中国留学生的名校梦不再高不可攀。不论学生是怎样的背景和成绩，鹿名国际都可以通过其三条成熟和独特的留学申请产品线（世界名校直通车、世界名校直通车、名校教授培养计划）将学生保送到最顶尖的名校，并在留学后的校园生活中为学生提供系统性的学术辅导服务（学业管家）。', ],
        visible: false,
        link: 'about',
    },
    {
        imgLink: 'bg.jpg',
        description: ['鹿名国际凭借着多年以来独有的世界顶尖大学教授、招生官、学校管理层以及名校学长学姐资源，为学生的申请路上保驾护航。2016年至2018年，鹿名国际迅速扩张，名校保录业务不断扩展到了欧洲、澳大利亚、加拿大、新加坡和香港等地。', ],
        visible: false,
        link: 'about',
    },
    {
        imgLink: 'index/12.jpeg',
        description: ['自2014年以来鹿名国际已帮助学员斩获600+世界顶尖院校录取。随着口碑逐渐扩大，企业家子女，众多精品教育机构等选择与我们合作，但由于一直秉承小而精的运营方式和高质量服务理念，鹿名国际业务在年年增长的趋势下，每年限制最多50名本科生和80名研究生的世界名校申请名额。', ],
        visible: false,
        link: 'about',
    },
    {
        imgLink: 'index/13.jpeg',
        description: ['目前鹿名国际学员已遍布包括哈佛、耶鲁、牛津、剑桥、斯坦福、沃顿、康奈尔、及哥伦比亚等世界知名大学。对于大多数学生，我们将申请到世界顶尖名校的概率从10%提升到约90%，这也进一步证明了鹿名国际留学申请业务的保证性。在确保服务结果的同时，我们也始终以98%的用户满意度为学员服务。', ],
        visible: false,
        link: 'about',
    },
]);
...
const teamData = ref([{
        ...
        description: 'Patrick老师坚信力量来源于知识，在申请服务过程不断激发学生的潜能，挖掘学生的亮点。让学生在压力极大的申请过程中不断提高自己能力和自我认知...',
        visible: false,
    },
    {
        name: 'Andy Xue',
        ...
        description: 'Andy老师坚信留学申请过程不是困惑的或者令人沮丧的。事实上，如果用聪明的方法，这个过程将是学生拥有的一个绝佳的机会，去帮助他们提高自我认知和自信。',
        visible: false,
    },
    {
        name: 'Alex Gu',
...
const professorData = ref([{
    ...
    },
    {
        ...
        description: '李贝贝教授现任卡内基梅隆大学(Carnegie Mel-IonUniversity）约翰.海因茨管理学院、计算机院与商学院Anna Loomis McCandless讲席教授，副教授与博士生导师。为纽约大学斯特恩商学院信息系统与管理学博士学位。专注于研究信息科技与个体行为、组织和社会的关系并，利用大规模历史和实验数据进行相关决策分析和政策制定。她的研究领域涉及智慧城市和医疗移动和物联网新零售，人工智能与大数据金融社交媒体和搜索引擎全渠道智能营销等等多篇论文发布在Managenment Science、Marketing Science、Information Systems Research和MIS Quarterly等顶级研究与商业期刊，以及一些经济，市场营销， 信息科学和计算科学的顶级会议上。并有多篇研究成果获国际大奖',
    },
    {
        name: '张庆全客座教授',
        title: [
        ...
        description: '张庆全博士是美国伊利诺伊大学香槟分校商学院客座教授。美国 Zacks 投资基金公司基金经理,美国 CFA 会芝加哥分会委员会委员，美国华人交易与投资协会芝加加哥分会会长，长期从事金融科技领域的研究，包括人工智能与大数据研究工作。已经在国际上发表了50多篇SCI论文，绝大部分是第一作者；张庆全博士毕业于美国芝加哥大学主攻方向金融，量化投资和创业学。张博士足美国电子电气工程协会高级会员( EEE Senior Member)，美国特许金分析( ChartedFinancial Analyst)',
    },
    {
        name: 'Scott Meadow教授',
        title: [
        ...
        description: 'Scott Meadow是私股权行业的主要投资者,理查德J.戴利奖的获得者。自2000年以来担任芝加哥大学布斯色学院的创业临床教授。2002年-2005年荣获菲尼克斯奖，2010年教师优秀奖。他被《商业周刊》的“最佳商学院指南”指定为该国杰出的企业家教授之一。为8000名学生教授创业融资和私募股权、商业化创新(他创建的)、新的风脸挑战，并担任风险投资竞争的教员顾问。是四家风险投资和私募股权公司合伙人，批准了数百项股权融资，亲自领导、发起或创建了60多项投资，被 Venture One 认可为业内杰出的医疗投资者之一。',
    },
]);
...
const changeSlideOfferType = function (type: string, index: number) {
